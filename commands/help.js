    ...
    try {
      ...
      commands.forEach((command) => {
        if (
          ...
          command.nameLocales[interaction.locale]
        )
          ...
          message =
            ...
            command.nameLocales[interaction.locale] +
            '** - ' +
            command.descriptionLocales[interaction.locale];
        else if (
          inputCommand === command.name &&
          !command.nameLocales[interaction.locale]
        )
      ...
      if (message === '') {
        embedColor = 0xff0000;
        message = locales[interaction.locale] ?? {
          commandNotFound: 'Cannot find any information about **',
        };
      ...
      commands.forEach((command) => {
        message +=
          '\n**' +
          `${command.nameLocales[interaction.locale] ?? command.name}` +
          '** - ' +
          (command.descriptionLocales[interaction.locale] ??
            command.description);
      });
